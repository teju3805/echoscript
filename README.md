...
- **Both LLM vendors retire model ids on a schedule.** Groq decommissioned `llama-3.3-70b-versatile` in August 2026 and Google retired `gemini-2.5-flash`; each returned a 404 mid-deployment. Both adapters now ask the provider's `/models` endpoint what it currently serves and retry once with a live id, caching the answer.
